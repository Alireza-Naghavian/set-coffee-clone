...
import { BasedPriceType, BasedRateStartType, FilteredCategoryType } from "@/types/Filter.type";
    ...
    <>
      <div
        ...
        <div
          ...
          />
          <BasedRateStart  setStars={setStars} />
        </div>
      </div>
      <div
...
const BasedRateStart = ({setStars}:BasedRateStartType) => {
  return (
  ...
  {[5,4,3,2,1].map((stars)=>{
      return  <div key={stars} className="flex w-full items-center group" onClick={()=>setStars(stars)} >
    ...
    <div className="mr-auto">
      ...
      <FaPlus   className=" rounded-xl text-xl    " />
      </Badge>
    </div>
  </div>
  })}
