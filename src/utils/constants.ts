
export const customeBlurDataURL =
  "data:image/webp;base64,";
  export const subMenuTitles = [
    { label: "Specialty coffee", href: `/categories?categoryId=667fed788adb606b5273c865` },
    { label: "World Class Specialty", href: `/categories?categoryId=667fed428adb606b5273c859` },
    { label: "Premium Coffee", href: `/categories?categoryId=667fed2b8adb606b5273c852` },
    { label: "Commercial Coffee", href: `/categories?categoryId=667fed338adb606b5273c856` },
    { label: "Coffee Capsule", href: `/categories?categoryId=667fed928adb606b5273c86b` },
    { label: "Italian Passion", href: `/categories?categoryId=667fed828adb606b5273c868` },
  ];
export const optionValues = [
  {
    label: "بر اساس جدید ترین",
    value: "latest",
  },
  {
    label: "بر اساس قدیمی ترین",
    value: "earliest",
  },
  {
    label: "بر اساس ارزان ترین",
    value: "lower_price",
  },
  {
    label: "بر اساس گرانترین",
    value: "expensive",
  },
];
export const roleOptions = [
  {
    label: "ادمین",
    value: "ADMIN",
  },
  { label: "کاربر عادی", value: "USER" },
];
export const subUserMenu = [
  { href: "/my-account", label: "پیشخوان" },
  { href: "/my-account/orders", label: "سفارش ها" },
  { href: "/my-account/details", label: "جزئیات حساب" },
  { href: "/my-account/tickets", label: "تیکت ها" },
  { href: "/my-account/wishlist", label: "لیست علاقه مندی ها" },
];
export const UserPanelAside = [
  { href: "/my-account", label: "پیشخوان" },
  { href: "/my-account/orders", label: "سفارش ها" },
  { href: "/my-account/details", label: "جزئیات حساب" },
  { href: "/my-account/wishlist", label: "لیست علاقه مندی ها" },
  { href: "/my-account/tickets", label: "تیکت ها" },
  { href: "/my-account/logout", label: "خروج" },
];
export const commentStatus = [
  { label: "تایید کردن", value: true },
  { label: "رد کردن", value: false },
];

export const priorityValues = [
  { label: "عادی", value: "1" },
  { label: "مهم", value: "2" },
  { label: "بسیار مهم", value: "3" },
];

export const ticketStatus = [
  {
    cond: {
      isPending: true,
      isAnswered: false,
      isOpen: true,
    },
    className: "bg-main_brown",
  },
  {
    cond: {
      isPending: false,
      isAnswered: true,
      isOpen: true,
    },

    className: "bg-green-500",
  },
  {
    cond: {
      isPending: false,
      isAnswered: true,
      isOpen: false,
    },

    className: "bg-red-400 ",
  },
];

export const ticketOptions = [
  {
    label: "باز کردن",
    value: true,
  },
  { label: "بستن", value: false },
];
type OptionSelectionType = {
  label: string;
  value: string;
  optionalItem?: boolean;
}[];
export const productSelectionOption: OptionSelectionType = [
  {
    label: "توضیحات",
    value: "desc",
  },
  { label: "مشخصات", value: "moreDetail" },
  { label: "نظرات", value: "comments", optionalItem: true },
];
export const adminCharts:OptionSelectionType =[
  {label:"نرخ فروش روزانه",value:"sales"},
  {label:"نرخ رشد وب سایت",value:"growth"}
] 


export const initialFilterAmount = {
  minPriceInit: 0,
  maxPriceInit: 10_000_000,
  sortInit: "latest",
  starsInit: 5,
};
